...
import com.eggloop.flow.sampler.LatinHypercubeSampler;
...
//        ((G[Tl_1, Tu_1] ((flow <= Theta_1 & (G[Tl_2, Tu_2] (flow <= Theta_2)) )))  | (F[Tl_3, Tu_3] flow >= Theta_3))
    ...
    //    (G[Tl_1, Tu_1] ((flow <= Theta_1 & G[Tl_2, Tu_2] (flow <= Theta_2) ))  | F[Tl_3, Tu_3] (flow >= Theta_3))
        ...
        ObjectiveFunction function = point -> {
            ...
            double[] value1 = computeAverageRobustness(ds2Times, normal_model, variables, formula, concatenate(point,new double[]{lb[point.length]}));
            double[] value2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, concatenate(point,new double[]{lb[point.length]}));
        ...
        GpoResult optimise;
        double[] lbU=IntStream.range(0,lb.length-1).mapToDouble(i->0).toArray();
        double[] ubU=IntStream.range(0,ub.length-1).mapToDouble(i->1).toArray();
...
//        double[] p2u2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, vv);
//        System.out.println(Arrays.toString(p2u2));
//        System.out.println(p1u1[0] - p2u2[0]);
//        System.out.println(Math.abs((p1u1[0] - p2u2[0]) / (3 * (p1u1[1] + p2u2[1]))));
////
...
//                x[0]=Math.max(0,x[0]);
//                x[1]=Math.max(0,x[1]);
//                x[0]=Math.min(100,x[0]);
//                x[1]=Math.min(100,x[1]);
...
//                    return 10;
//                }
//                double[] p1u1 = computeAverageRobustness(ds2Times, normal_model, variables, formula, new double[]{x[0],x[1],0});
//                double[] p2u2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, new double[]{x[0],x[1],0});
//                return Math.abs((p1u1[0]-p2u2[0])/(3*(p1u1[1]+p2u2[1])));
//            }
//        };
//
//        double[] x = new double[]{0,20};
//        double rhobeg = 100;
//        double rhoend = 1E-3;
//        int iprint = 1;
//        int maxfun = 3500;
//        CobylaExitStatus result = Cobyla.FindMinimum(calcfc, 2, 4, x, rhobeg, rhoend, iprint, maxfun);
//        System.out.println(x);
//        double lb = 0;
//        double ub = 100;
//        ObjectiveFunction function = point -> {
//            point[1] = point[0] + point[1] * (1 - point[0]);
//            final double[] p = point;
//            point = IntStream.range(0, point.length).mapToDouble(i -> lb + p[i] * (ub - lb)).toArray();
//            double[] value1 = computeAverageRobustness(ds2Times, normal_model, variables, formula, new double[]{point[0], point[1], 0});
//            double[] value2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, new double[]{point[0], point[1], 0});
//
//            double abs = Math.abs((value1[0] - value2[0]) / (3 * (value1[1] + value2[1])));
//            System.out.println(abs);
//            return abs;
...
//        //kernelGP.setHyperarameters(new double[]{Math.log(normal_model.length + 1), 0.4});
//        options.setKernelGP(kernelGP);
//        options.setGridSampleNumber(10);
//        gpo.setOptions(options);
//        GpoResult optimise;
//
//        optimise = gpo.optimise(function, new double[]{0,0}, new double[]{1,1});
//        System.out.println(Arrays.toString(optimise.getSolution()));
//        final double [] vector =optimise.getSolution();
//        vv = IntStream.range(0, vector.length).mapToDouble(i -> lb + vector[i] * (ub - lb)).toArray();
//        vv=concatenate(vv,new double[]{0});
//        System.out.println(Arrays.toString(vv));
//        p1u1 = computeAverageRobustness(ds2Times, normal_model, variables, formula, vv);
//        //System.out.println(Arrays.toString(p1u1));
//        p2u2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, vv);
//        //System.out.println(Arrays.toString(p2u2));
//        System.out.println("-->"+Arrays.toString(vv));
//        vv[2]=(p1u1[0]+p2u2[0])/2;
//        System.out.println("-->"+Arrays.toString(vv));
//        p1u1 = computeAverageRobustness(ds2Times, normal_model, variables, formula, vv);
//        System.out.println(Arrays.toString(p1u1));
//        p2u2 = computeAverageRobustness(ds2Times, ineffective_model, variables, formula, vv);
