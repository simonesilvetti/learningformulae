...
public class TESTARTICOLO2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++) {
            double[] vv = ComputeAverage.prob(ds2Times, normal_model, ineffective_model, pop.getFormula(i), pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound});
            System.out.println(Arrays.toString(vv));
            p1u1[iterationIndex][i] = ComputeAverage.computeProb(ds2Times, normal_model, variables, pop.getFormula(i), vv);
            p2u2[iterationIndex][i] = ComputeAverage.computeProb(ds2Times, ineffective_model, variables, pop.getFormula(i), vv);
        ...
        if (timePar.length + thrsPar.length < 11) {
            ...
            double[] vv = ComputeAverage.prob(ds2Times, normal_model, ineffective_model, bformula, pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound});
            p1u1[iterationIndex][fitnessOrdered[0]] = ComputeAverage.computeProb(ds2Times, ineffective_model, variables, bformula, vv);
            p2u2[iterationIndex][fitnessOrdered[0]] = ComputeAverage.computeProb(ds2Times, normal_model, variables, bformula, vv);
...
//            p2u2[iterationIndex][fitnessOrdered[0]] = smc(pop, bformula,ds2Times, normal_model);
        ...
        while (terminationIndex < termination) {
            ...
            for (int k = 0; k < numParents; k++) {
                ...
                double[] vv = ComputeAverage.prob(ds2Times, normal_model, ineffective_model, pop.getFormulaNewGeneration(k), pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound});
                ...
                //System.out.println("11111111111111111111111");
                double[] p1u1d = ComputeAverage.computeProb(ds2Times, ineffective_model, variables, pop.getFormulaNewGeneration(k), vv);
                double[] p2u2d = ComputeAverage.computeProb(ds2Times, normal_model, variables, pop.getFormulaNewGeneration(k), vv);
...
//                double[] p2u2d = smc(pop, pop.getFormulaNewGeneration(k),ds2Times, normal_model);
                ...
                vv = ComputeAverage.prob(ds2Times, normal_model, ineffective_model, pop.getFormulaNewGeneration(k + 1), pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound});
                for (int i = 0; i < vv.length; i++) {
                    if (Double.isNaN(vv[i])) {
                        vv[i] = 0;
                    }
                ...
                p1u1d = ComputeAverage.computeProb(ds2Times, ineffective_model, variables, pop.getFormulaNewGeneration(k + 1), vv);
                p2u2d = ComputeAverage.computeProb(ds2Times, normal_model, variables, pop.getFormulaNewGeneration(k + 1), vv);
...
//                p2u2d = smc(pop, pop.getFormulaNewGeneration(k + 1),ds2Times, normal_model);
                ...
                if (pop.getFormulaNewGeneration(k) != parent1 && pop.getFormulaNewGeneration(k) != parent2) {
                    vv = ComputeAverage.prob(ds2Times, normal_model, ineffective_model, pop.getFormula(k), pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound});
                    //continuousOptimization(pop,variables, pop.getFormula(k), normal_model, ineffective_model,ds2Times);
                    p1u1[iterationIndex + 1][k] = ComputeAverage.computeProb(ds2Times, ineffective_model, variables, pop.getFormula(k), vv);
                    p2u2[iterationIndex + 1][k] = ComputeAverage.computeProb(ds2Times, normal_model, variables, pop.getFormula(k), vv);
//
...
//        System.out.println("u2bestFormula= " + u2bestFormula);
        ...
        //double[] vv = ComputeAverage.prob(ds2Times,normal_model,ineffective_model, bestFormula, pop,new double[]{GeneticOptions.min_time_bound,GeneticOptions.max_time_bound});
