...
public class FinalTest {
    public static void main(String[] args) {
        ...
        //Ottimizzazione robustness
        double[] bestPar2 = continuousOptimizationRobustnessWCobyla(pop2,variables, formula, normal_model, ineffective_model, ds2Times);
        ...
        System.out.println("Optimized_robustness: " + Arrays.toString(p1u1));
        System.out.println("Optimized_robustness: " + Arrays.toString(p2u2));
...
//        popgen.setParameters(parsTrshld, bestPar);
    ...
    static double[] continuousOptimizationRobustnessWCobyla(FormulaPopulation popgen,String[] variablesUnique, Formula formula, double[][] positiveDataset, double[][] negativeDataset, double[] times) {
        double[] timeBounds = new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound};
        String[] bounds = formula.getTimeBounds();
        String[] variables = formula.getVariables();
//        String[] trshld = formula.getThresholds();
...
//        System.arraycopy(ubThrshld, 0, ub, bounds.length, ubThrshld.length);
        ...
        ObjectiveFunction function = new FindBestParamterswithCobylaHelp(variablesUnique, positiveDataset, negativeDataset, formula, timeBoundsLb, timeBoundsUb);
        GPOptimisation gpo = new GPOptimisation();
        GpoOptions options = new GpoOptions();
        options.setGridSampler(new LatinHypercubeSampler(2,10));
        options.setMaxIterations(150);
        options.setHyperparamOptimisation(false);
        options.setUseNoiseTermRatio(true);
        options.setNoiseTerm(3);
        KernelRBF kernelGP = new KernelRBF();
        ...
        optimise= gpo.optimise(function, lbThrshld, ubThrshld);
