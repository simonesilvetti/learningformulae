...
import com.eggloop.flow.numeric.optimization.cobyla2.CobylaExitStatus;
...
public class TestReading2 {

    public static void main(String[] args) {
        Random ran = new Random(1);
        double[][] ds2SpatialValues = Utils.readMatrixFromFile(com.eggloop.flow.simhya.simhya.matlab.TestReading.class.getClassLoader().getResource("data/calin/ds2Trajectories").getPath());
        double[] ds2Labels = Utils.readVectorFromFile(com.eggloop.flow.simhya.simhya.matlab.TestReading.class.getClassLoader().getResource("data/calin/ds2Labels").getPath());
        double[] ds2Times = Utils.readVectorFromFile(com.eggloop.flow.simhya.simhya.matlab.TestReading.class.getClassLoader().getResource("data/calin/ds2Times").getPath());
...
//        formula = pop2.loadFormula(FF, pars, vv);
        ...
        double[] bestPar2 = continuousOptimizationRobustnessCobyla(pop2, variables, formula, normal_model, ineffective_model, ds2Times);
...
//            double[] nP= IntStream.range(0, lb.length).mapToDouble(k -> k%2==1&&k<bounds.length ? ran[k-1]+(ub[k]-ran[k-1])*Math.random():k).toArray();
    ...
    static void continuousOptimizationCobyla(FormulaPopulation popgen, Formula formula, BreathSimulator simulator1, BreathSimulator simulator2, double Tf, int samples) {
        double[] timeBounds = new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound};
        String[] bounds = formula.getTimeBounds();
        String[] variables = formula.getVariables();
        String[] trshld = formula.getThresholds();
    ...
    static double[] continuousOptimizationRobustnessCobyla(FormulaPopulation popgen, String[] variablesUnique, Formula formula, double[][] positiveDataset, double[][] negativeDataset, double[] times) {
        double[] timeBounds = new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound};
        String[] bounds = formula.getTimeBounds();
        String[] variables = formula.getVariables();
        String[] trshld = formula.getThresholds();
